...
// Prefer NEWS_DB_PATH. On Vercel the project dir is read-only, so default to /tmp
// (ephemeral — use a persistent store or external path for production durability).
function resolveDbPath() {
  if (process.env.NEWS_DB_PATH) return process.env.NEWS_DB_PATH;
  if (process.env.VERCEL) {
    console.warn(
      '[db] VERCEL detected without NEWS_DB_PATH; using /tmp/news_archive.db (ephemeral across cold starts)'
    );
    return path.join('/tmp', 'news_archive.db');
  }
  return path.join(process.cwd(), 'news_archive.db');
}
