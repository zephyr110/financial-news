...
export async function getIndustryTrend(hoursBack = 24) {
  ...
  // Group by 2-hour buckets and industry
  const buckets = new Map();
  for (const row of result.rows) {
    let industries = [];
    try { industries = JSON.parse(row.industries); } catch { continue; }
    const dt = new Date(row.published_at);
    // Round to 2-hour bucket
    dt.setMinutes(0, 0, 0);
    dt.setHours(Math.floor(dt.getHours() / 2) * 2);
    const key = dt.toISOString().slice(0, 16); // "2026-07-25T10:00"
  ...
  // Convert to { time, [industry]: count } format for Recharts
  const data = Array.from(buckets.entries())
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([time, indMap]) => ({
      time: time.slice(11), // "10:00"
...
export async function saveEventThreads(threads) {
  const db = await getDb();
  // Clear old threads
  await db.execute('DELETE FROM event_threads');
...
export async function getEventThreads(hoursBack = 24) {
  ...
  return result.rows.map(r => ({
    ...r,
    news_ids: safeParseJson(r.news_ids),
    industries: safeParseJson(r.industries),
    watch_points: safeParseJson(r.watch_points),
  }));
}
